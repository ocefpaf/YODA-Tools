from DataLoader.domain.work import yodatool as ydt_base
...
class DataColumn(ydt_base.BaseDataColumn):
    def __init__(self, dc=None):
        if isinstance(dc,list):
            ...
            self.StatusCV = dc[10]
            self.SampledMediumCV = dc[11]
            self.IntendedTimeSpacing = dc[12]
            self.IntendedTimeSpacingUnit = dc[13]
            self.TimeAggregationInterval = dc[14]
            self.TimeAggregationIntervalUnitCode = dc[15]
            self.AggregationStatisticCV = dc[16]
            self.CensorCodeCV = dc[17]
            self.QualityCodeCV = dc[18]
