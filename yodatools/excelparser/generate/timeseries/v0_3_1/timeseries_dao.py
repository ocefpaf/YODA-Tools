...
import yodatools.excelparser.generate.timeseries.v0_3_1.timeseries_models as model
import math
...
class TimeseriesXlDao(BaseXldao):
    ...
    def get_people_dictionary(self):
        ...
        if sheet is None:
            return None
        num = int(xw.Range(sheet,"NumPeople").value)
        if num <= 0:
            return None
    ...
    def get_all_organizations(self):
        ...
        if sheet is None:
            return None
        num = int(xw.Range(sheet,'NumOrganizations').value)
        if num <= 0:
            return None
    ...
    def get_all_authorlists(self):
        ...
        if sheet is None:
            return None
        NumAuthors = int(xw.Range(sheet,'NumAuthors').value)
        if NumAuthors <= 0:
            return None
    ...
    def get_all_samplingfeatures(self):
        ...
        NumSamplingFeatures = int(xw.Range(sheet,'NumSamplingFeatures').value)
        if NumSamplingFeatures > 0:
            samplingfeatures = xw.Range(sheet,'SamplingFeatures').value
            sfResults = []
            for sf in samplingfeatures[0:NumSamplingFeatures]:
    ...
    def get_all_sites(self):
        ...
        NumSites = int(xw.Range(sheet,'NumSites').value)
        if NumSites > 0:
            samplingfeatures = xw.Range(sheet,'SamplingFeatures').value
            sResults = []
            for sf in samplingfeatures[0:NumSites]:
                sf.insert(8,ElevationDatum)
                sf_obj = model.SamplingFeature(sf[0:9])
                s = [sf[11],sf[12],sf[13],sf_obj,sr_obj]
    ...
    def get_all_spatialoffsets(self):
        ...
        if sheet is None:
            return None
        num = int(xw.Range(sheet,'NumSpatialOffsets').value)
        if num <= 0:
            return None
        SpatialOffsets = xw.Range(sheet,'RelatedFeatures').value
        soResults = []
        for so in SpatialOffsets[0:num]:
    ...
    def get_all_relatedfeatures(self):
        ...
        if sheet is None:
            return None
        num = int(xw.Range(sheet,'NumRelatedFeatures').value)
        if num <= 0:
            return None
        rfResults = []
        RelatedFeatures = xw.Range(sheet,'RelatedFeatures').value
        for rf in RelatedFeatures[0:num]:
    ...
    def get_all_methods(self):
        ...
        if sheet is None:
            return None
        NumMethods = int(xw.Range(sheet,'NumMethods').value)
        if NumMethods <= 0:
            return None
        Methods = xw.Range(sheet,'Methods').value
        mResults = []
        for m in Methods[0:NumMethods]:
    ...
    def get_all_variables(self):
        ...
        if sheet is None:
            return None
        NumVariables = int(xw.Range(sheet,'NumVariables').value)
        if NumVariables <= 0:
            return None
        Variables = xw.Range(sheet,'Variables').value
        vResults = []
        for v in Variables[0:NumVariables]:
    ...
    def get_all_processinglevels(self):
        ...
        if sheet is None:
            return None
        NumProcessingLevels = int(xw.Range(sheet,'NumProcessingLevels').value)
        if NumProcessingLevels <= 0:
            return None
        plResults = []
        ProcessingLevels = xw.Range(sheet,'ProcessingLevels').value
        for p in ProcessingLevels[0:NumProcessingLevels]:
    ...
    def get_all_units(self):
        ...
        if sheet is None:
            return None
        NumUnits = int(xw.Range(sheet,'NumUnits').value)
        if NumUnits <= 0:
            return None
        uResults = []
        Units = xw.Range(sheet,'Units').value
        for u in Units[0:NumUnits]:
    ...
    def get_all_datacolumns(self):
        ...
        if sheet is None:
            return None
        NumDataColumns = int(xw.Range(sheet,'NumDataColumns').value)
        if NumDataColumns <= 0:
            return None
        datacloumnResults = []
        DataColumns = xw.Range(sheet,'DataColumns').value
        for d in DataColumns[0:NumDataColumns]:
