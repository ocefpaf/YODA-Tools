...
class YamlPrinter():
    ...
    def get_header(self, filetype):
        ...
        yoda_header += 'DateCreated: "{0}", '.format(datetime.datetime.strftime(datetime.datetime.now(), '%Y-%m-%d'))  # noqa
        yoda_header += 'DateUpdated: "{0}"'.format(datetime.datetime.strftime(datetime.datetime.now(), '%Y-%m-%d'))  # noqa
        yoda_header += '}\n'
    ...
    def print_objects(self, apiobjs, ind=0):
        ...
        for obj in apiobjs:
            ...
            # add this try/except block to make sure the inherited objects'
            # dictionaries have all the metadata
            self.fill_dict(obj)
        ...
        for k, v in d.items():
            ...
            if v is None:
                ...
                if '*' in v:
                    ...
                    final_string += '{}: "{}", '.format(k, v)
            elif isinstance(v, datetime.datetime) or isinstance(v, datetime.date):  # noqa
                final_string += '{}: "{}", '.format(k, v.strftime('%Y-%m-%d %H:%M'))  # noqa

        final_string = '{}}}\n'.format(final_string[:-2])

        return final_string

    def add_to_db(self, objname, fname, data):

        if objname in data:
            # check to see if this is an inherited object
            if data[objname][0].__class__ != data[objname][0]._sa_instance_state.key[0]:  # noqa
                fname.write(self.handle_inheritance(apiobjs=data[objname]))
            else:
                fname.write(self.print_objects(data[objname]))
    ...
    def print_yoda(self, out_file, data):
        ...
        with open(out_file, 'w') as yaml_schema_file:
            ...
            # samplingfeatures: Not explicitly printed,
            # should be included in sites and specimen objects
            # sites
            ...
            # result Not explicitly printed,
            # should be included in measurement or timeseries results
            ...
            self.add_to_db('measurementresultvalueannotations', yaml_schema_file, data)  # noqa
